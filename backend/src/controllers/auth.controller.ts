...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.create({ email, password: hashedPassword, username: username });
  ...
  try {
    ...
    return res.status(201).json({"username": user.username, "email": user.email, "role": user.role});
  } catch (error) {
